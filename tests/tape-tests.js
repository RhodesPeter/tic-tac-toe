...
test('Testing that winner() returns the correct winning player', function(t){
  var board = [' ', 'O', 'O', '.', '.', '.', '.', '.', ' '];
  t.notOk(game.winner(board)[0], 'false is returned when pattern is not matched in patterns_3');
  t.equals(game.winner(board)[1], null, 'test passed: O in the winner');
  t.end();
});
...
test('Testing that winner() returns the correct winning player', function(t){
  ...
  t.equals(game.winner(board)[1], 'O', 'test passed: O in the winner');
  t.end();
});
...
test('Testing that winner() returns the correct winning player', function(t){
  ...
  t.equals(game.winner(board)[1], 'X', 'test passed: X in the winner');
  t.end();
});
